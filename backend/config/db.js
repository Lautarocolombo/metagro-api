...
async function initDb() {
  ...
  await pool.query('SELECT count(*) as total FROM home_content').then(({ rows }) => {
  ...
  await pool.query('SELECT count(*) as total FROM translations').then(({ rows }) => {
    const transCount = parseInt(rows[0].total, 10);
    if (transCount === 0) {
      return pool.query(`
