...
for (const seed of seedProducts) {
  ...
  // Buscar el producto con el mismo ID usando regex
  ...
  if (existingMatch) {
    ...
    // Buscar el último producto y agregar después
    const lastProductEnd = productsStr.lastIndexOf('  },');
    if (lastProductEnd !== -1) {
      const insertPos = lastProductEnd + 4;
      productsStr = productsStr.slice(0, insertPos) + '\n' + newProduct + productsStr.slice(insertPos);
